  ...
  internal sealed class Support
  {
    ...
    private static string CorrectNumberDecimalSeparator(string inNumber, Support.DecimalSeparatorType separatorType)
    {
      ...
      try
      {
        ...
        str2 = businessObject[].Item((object) TaxDeterminationLoader.Resources.Resources.QuerySboForDecimalSeparator_FieldName).Value.ToString();
        oldValue = businessObject[].Item((object) TaxDeterminationLoader.Resources.Resources.QuerySboForThousandSeparator_FieldName).Value.ToString();
      }
      catch (Exception ex)
      {
        throw ex;
    ...
    private static string CorrectNumberDecimalSeparatorFromGridToWindowsFormat(string inNumber)
    {
      ...
      try
      {
        Recordset businessObject = (Recordset) Support._oCompany.GetBusinessObject(BoObjectTypes.BoRecordset);
        businessObject.DoQuery(TaxDeterminationLoader.Resources.Resources.QuerySboForDecimalAndThousandSeparator);
        oldValue1 = businessObject[].Item((object) TaxDeterminationLoader.Resources.Resources.QuerySboForDecimalSeparator_FieldName).Value.ToString();
        oldValue2 = businessObject[].Item((object) TaxDeterminationLoader.Resources.Resources.QuerySboForThousandSeparator_FieldName).Value.ToString();
      }
      catch (Exception ex)
      {
        throw ex;
    ...
    {
      string str = ((IEditText) Support._oForm[].Item((object) editBoxName)[]).String;
    ...
    internal static bool GetMatrixEditBoxContents(string columnName, int columnPos, int rowIndex, string textToGet)
    {
      ...
      {
        if (columnName != "")
          column = Support._oMatrix[].Item((object) columnName);
      }
      else
        column = Support._oMatrix[].Item((object) columnPos);
      EditText editText = (EditText) column.Cells.Item((object) rowIndex)[];
    ...
    internal static Decimal GetMatrixItemFromCurrentRowAsDecimal(string columnName, int rowIndex)
    {
      ...
      try
      {
        Column column = Support._oMatrix[].Item((object) columnName);
        EditText editText = (EditText) column.Cells.Item((object) rowIndex)[];
    ...
    {
      return RS[].Item((object) pos).Value.ToString();
    }

    public string getSAPFComboValue(string form, string item, string type)
    {
      Support._oItem = Support._sboApp.Forms.GetForm(form, 1)[].Item((object) item);
      SAPbouiCOM.ComboBox comboBox = (SAPbouiCOM.ComboBox) Support._oItem[];
    ...
    {
      Support._oItem = Support._sboApp.Forms.Item((object) formUID)[].Item((object) comboUID);
      SAPbouiCOM.ComboBox comboBox = (SAPbouiCOM.ComboBox) Support._oItem[];
      if (comboBox.Selected == null)
        return "";
      if (!(type != "value"))
    ...
    {
      Support._oItem = Support._sboApp.Forms.Item((object) formUID)[].Item((object) editUID);
      return ((IEditText) Support._oItem[]).Value;
    }
    ...
    private static void SetoMatrix(string matrixName, bool specific)
    {
      if (!specific)
      {
        Support._oMatrix = (Matrix) Support._oForm[].Item((object) matrixName);
      }
      else
      {
        try
    ...
    {
      Support._oItem = Support._sboApp.Forms.Item((object) formUID)[].Item((object) editUID);
      ((IEditText) Support._oItem[]).String = val;
    }
